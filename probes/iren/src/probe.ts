...
const PAGES: { url: string; label: string }[] = [
  { url: 'https://iren.com/', label: 'home' },
  { url: 'https://iren.com/ai-cloud-services', label: 'ai-cloud' },
];
...
export async function runIrenProbe(outDir: string): Promise<EvidenceObject[]> {
  ...
  const outPath = join(outDir, `${yyyy}-${mm}-${dd}`, hh, 'iren.json');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  runIrenProbe(outDir)
    .then((records) => {
      console.log(`IREN probe complete: ${records.length} records written`);
    })
    .catch((err) => {
      console.error('IREN probe failed:', err);
      process.exit(1);
    });
}
